...
function onData(event) {
    ...
    // Updated to support 17 samples per packet (1 kHz @ 17ms batches)
    if (batchCount === 0 || batchCount > 17) {
        console.warn("Invalid batch count:", batchCount);
        return;
    }

    // Verify packet size matches expected
    const expectedSize = 1 + (batchCount * 14);
    if (view.byteLength < expectedSize) {
        console.warn("Packet too small:", view.byteLength, "expected:", expectedSize);
        return;
    }
